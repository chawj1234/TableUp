...
모든 조작은 Claude Code 에서 **자연어** 로 이뤄집니다.

### 기본
```
You: 이 PDF 에서 표 뽑아줘 — ./report.pdf
Claude Code: [Upstage Document Parse auto 모드 호출 → .tableup/report/ 생성 → 요약 안내]
```
...
Claude Code: [macOS mdfind / Linux locate — Spotlight 인덱스 활용]
```
...
### 고품질 강제 (모든 페이지에 enhanced 적용)
```
You: 이 보고서는 전부 고품질로 뽑아줘 — ./critical.pdf
```

### 페이지 범위 지정 (PDF 만)
```
You: ./large.pdf 의 p.12~15 재무제표만 뽑아줘
```

### 뽑은 후 분석
```
You: 방금 뽑은 신용증가율 표에서 전년 대비 변화율 계산해줘
Claude Code: [.tableup/<파일명_stem>/t00_p3_*.csv 로드 → pandas 분석]
```
...
- 전역 캐시 `~/.cache/tableup/<sha256>_<mode>.json` → 같은 파일·모드 재처리 시 API 호출 0
...
상세: [`evals/results/2026-04-14/report.md`](evals/results/2026-04-14/report.md)  
시나리오 스펙: [`01`](evals/scenario-01-complex-table.md) · [`02`](evals/scenario-02-chart-to-data.md) · [`03`](evals/scenario-03-hwp-derived.md)
