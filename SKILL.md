...
1. 사용자에게서 **PDF 경로**와 **페이지 범위(선택)** 를 확인한다. 경로가 모호하면 아래 "파일 경로 해결" 절차를 먼저 수행한다.
...
3. 스크립트를 실행한다:
   ...
   ```
   옵션:
   - `--search <키워드>`: CWD·~/Downloads·~/Desktop·~/Documents 에서 부분 파일명으로 검색
...
사용자가 전체 경로 대신 부분 파일명·추상 표현을 줄 때 다음 순서로 해결한다:

1. **사용자 언급이 부분 파일명인 경우** (예: "금융안정 PDF", "AI 보고서"):
   - 먼저 Glob 도구로 다음 경로에서 `*<키워드>*.pdf` 를 검색한다:
     - `~/Downloads/**/*<키워드>*.pdf`
     - `~/Desktop/**/*<키워드>*.pdf`
     - 현재 작업 디렉토리 `./**/*<키워드>*.pdf`
   - 또는 스크립트 내장 검색 사용: `python scripts/tableup.py --search "<키워드>"`

2. **"방금", "최근", "다운받은"** 표현이 있으면:
   - `~/Downloads/` 내 최근 수정 PDF 3개를 나열하고 사용자에게 선택 요청한다.

3. **매칭 결과 처리**:
   - 0개 매칭: 경로를 명시적으로 요청한다.
   - 1개 매칭: 확인 없이 바로 실행한다.
   - 2개 이상 매칭: 번호 리스트로 제시하고 선택을 받는다.

4. **전체 경로가 이미 주어진 경우**: 위 단계를 건너뛰고 바로 실행한다.
