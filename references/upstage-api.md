...
UpParse 이 사용하는 Upstage API 핵심만. 전체 명세는 [공식 에이전트 문서](https://console.upstage.ai/api/docs/for-agents/raw) 참조.
...
Sync 엔드포인트. 100페이지 제한. UpParse 은 100페이지 초과 시 클라이언트 측에서 chunk 분할 후 순차 호출한다.
...
| `mode` | UpParse 는 가장 정확한 옵션을 항상 사용 (CLI 노출 없음) |
